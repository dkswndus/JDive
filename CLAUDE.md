...
- **백엔드는 끝났다**(DB·마이그레이션, Google 로그인·세션·로그아웃·계정 삭제, 경험 CRUD, 공고 저장·중복 감지). 프론트엔드는 **로그인·세션·로그아웃·계정 삭제 화면, 오류 수집 원문 제거(Session Replay 없음), 경험 화면(수동 등록·수정·삭제)까지 끝났다**. 진행 상황은 `git log`가 기준이다.
- **남은 작업:** 공고 입력 화면(저장·중복 `409` 처리·목록·상세) → 완료 기준(스펙 §8) 점검, README·Google 설정 안내, PR·CI 확인·병합.
...
- 필드 길이·개수 제한은 스펙 §1.2 표를 먼저 확인한다(경험 활동 상한을 놓쳐 고친 적이 있다). 글자 수 초과만 `input_too_long`, 나머지 검증 실패는 `validation_error`다. 프론트엔드에도 같은 값이 있다(`lib/experiences.ts`의 `LIMITS`. 글자 수는 서버처럼 코드 포인트로 센다). 스펙을 바꾸면 백엔드와 프론트엔드를 함께 바꾼다.
